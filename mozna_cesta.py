...
for polygon in objekty:
    ...
    if (len(pruseciky) > 0):
        ty_spravne_body_polygonu = pridej_body_polygonu_v_polorovine(objekty, n_u)
        # toto mozna spojit jinak
        vysledne_body.append(pruseciky, ty_spravne_body_polygonu)


def pridej_body_polygonu_v_polorovine(objekty, n_u, polygon, point):
    c = - ((n_u[3] - n_u[1]) * n_u[0] + (n_u[0] - n_u[2]) * n_u[1])
    print(c)
    point = []
    for polygon in objekty:
...
(c < 0) and (x < 0)
    ...
    # u = (koncovy_bod[0] - zacatecni_bod[0], koncovy_bod[1] - zacatecni_bod[1])
    # n = (koncovy_bod[1] - zacatecni_bod[1], zacatecni_bod[0] - koncovy_bod[0])
    # p: (koncovy_bod[1] - zacatecni_bod[1]) * zacatecni_bod[0] + (zacatecni_bod[0] - koncovy_bod[0]) * zacatecni_bod[1] + c = 0
